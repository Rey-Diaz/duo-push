...
class DuoAuthenticator:
    ...
    def authenticate_user(self, username, auth_type='push', device=None, passcode=None):
        # auth_types are: push, sms, passcode
        # passcode includes sms passcode, hardware token, and OTP from Duo app
        if auth_type == 'passcode' and not passcode:
            return 'error: no passcode provided'
        uri = '/auth/v2/auth'
        params = {'username': username, 'factor': auth_type, 'async': '1'}
        if device:
